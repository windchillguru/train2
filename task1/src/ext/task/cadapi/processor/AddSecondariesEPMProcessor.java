package ext.task.cadapi.processor;
 ...
 * @author 段鑫扬
 */
public class AddSecondariesEPMProcessor extends DefaultObjectFormProcessor {
    private static final String CLASSNAME = AddSecondariesEPMProcessor.class.getName();
    ...
    public FormResult doOperation(NmCommandBean commandBean, List<ObjectBean> objectBeans) throws WTException {
        ...
        try {
            ...
            if (refObject != null && refObject instanceof EPMDocument) {
                epmDocument = (EPMDocument) refObject;
            }
            if (epmDocument == null) {
                throw new WTException("图纸为空");
            }
            ...
            for (File file : fileMap.keySet()) {
                ...
                ApplicationData applicationData = ApplicationData.newApplicationData(epmDocument);
                //附件
            ...
            addSecondaries(epmDocument, newItems);
     ...
     * @param newItems
     */
    private void addSecondaries(EPMDocument epmDocument, Map<Persistable, InputStream> newItems) throws WTException {
        if (epmDocument == null) {
            return;
        }
        Transaction tx = null;
