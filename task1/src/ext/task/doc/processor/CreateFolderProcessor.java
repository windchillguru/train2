...
public class CreateFolderProcessor extends DefaultObjectFormProcessor {
    ...
    public FormResult doOperation(NmCommandBean commandBean, List<ObjectBean> objectBeans) throws WTException {
        ...
        if (refObject instanceof WTDocument) {
            ...
            //所在文件夹
            Folder folder = FolderHelper.service.getFolder((FolderEntry) document);
            //获取当前文件夹路径
            folderPath = folder.getFolderPath();
        }
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(">>>> doOperation--->" + CLASSNAME + ",folderPath =" + folderPath);
        }
        ...
        if (PIStringUtils.hasText(folderName)) {
            ...
            try {
                ...
                if (folder != null) {
                    throw new WTException("文件夹已存在1111");
                }
